...
const renderProducts = async (url) => {
	try {
		...
		const desiccants = res.filter(item => item.subCategory?.link === 'desiccants');

		createProductCard(desiccants, '.protucts-content__list');
		updateBasketBgColor(getItem('basket'));
		renderCategoryFilter(plantsProtections);
		document.querySelector('.products-length').textContent = `Показано ${desiccants.length} товарів`;
...
manufacturerFilterContainer.addEventListener('change', async () => {
	try {
		...
		let filteredProducts = desiccants;
		if (selectedManufacturers.length > 0) {
			filteredProducts = desiccants.filter(item => selectedManufacturers.includes(item.manufacturer.id));
		}
...
// Сортировка 
document.querySelector('#products-sort').addEventListener('change', (e) => sortProducts(e, {subCategory: 'desiccants'}));