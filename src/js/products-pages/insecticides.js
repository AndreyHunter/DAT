...
const renderProducts = async (url) => {
	try {
		...
		const insecticides = res.filter(item => item.subCategory?.link === 'insecticides');

		createProductCard(insecticides, '.protucts-content__list');
		updateBasketBgColor(getItem('basket'));
		renderCategoryFilter(plantsProtections);
		document.querySelector('.products-length').textContent = `Показано ${insecticides.length} товарів`;
...
manufacturerFilterContainer.addEventListener('change', async () => {
	try {
		...
		let filteredProducts = insecticides;
		if (selectedManufacturers.length > 0) {
			filteredProducts = insecticides.filter(item => selectedManufacturers.includes(item.manufacturer.id));
		}
...
// Сортировка 
document.querySelector('#products-sort').addEventListener('change', (e) => sortProducts(e, {subCategory: 'fungicides'}));