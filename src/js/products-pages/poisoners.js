...
const renderProducts = async (url) => {
	try {
		...
		const poisoners = res.filter(item => item.subCategory?.link === 'poisoners');

		createProductCard(poisoners, '.protucts-content__list');
		updateBasketBgColor(getItem('basket'));
		renderCategoryFilter(plantsProtections);
		document.querySelector('.products-length').textContent = `Показано ${poisoners.length} товарів`;
...
manufacturerFilterContainer.addEventListener('change', async () => {
	try {
		...
		let filteredProducts = poisoners;
		if (selectedManufacturers.length > 0) {
			filteredProducts = poisoners.filter(item => selectedManufacturers.includes(item.manufacturer.id));
		}
...
// Сортировка 
document.querySelector('#products-sort').addEventListener('change', (e) => sortProducts(e, {subCategory: 'poisoners'}));