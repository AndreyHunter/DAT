...
const renderProducts = async (url) => {
	try {
		...
		const herbicides = res.filter(item => item.subCategory?.link === 'herbicides');

		createProductCard(herbicides, '.protucts-content__list');
		updateBasketBgColor(getItem('basket'));
		renderCategoryFilter(plantsProtections);
		document.querySelector('.products-length').textContent = `Показано ${herbicides.length} товарів`;
...
manufacturerFilterContainer.addEventListener('change', async () => {
	try {
		...
		let filteredProducts = herbicides;
		if (selectedManufacturers.length > 0) {
			filteredProducts = herbicides.filter(item => selectedManufacturers.includes(item.manufacturer.id));
		}
...
// Сортировка 
document.querySelector('#products-sort').addEventListener('change', (e) => sortProducts(e, {subCategory: 'herbicides'}));